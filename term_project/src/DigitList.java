public class DigitList {
    ...
    public static DigitList subDigitLists(int c, DigitList L1, DigitList L2) {
        // code here
        if ((L1 == null) && (L2 == null))
            return trimDigitList(digitize(c));
        else if (L1 == null)
            return subDigitLists(c, L2, null);
        else if (L2 == null) {
            if (L1.getDigit() < c) {
                int t = L1.getDigit() - c + 10;
                return trimDigitList(new DigitList(t, subDigitLists(1, L1.getNextDigit(), null)));
            }
            int t = L1.getDigit() - c;
            return trimDigitList(new DigitList(t, subDigitLists(0, L1.getNextDigit(), null)));
        } else {
            if (compareDigitLists(L1, L2) == -1) {
                DigitList tmp = L1;
                L1 = L2;
                L2 = tmp;
            }
            if (L1.getDigit() < L2.getDigit() + c) {
                int t = L1.getDigit() - L2.getDigit() + 10 - c;
                return trimDigitList(new DigitList(t, subDigitLists(1, L1.getNextDigit(), L2.getNextDigit())));
            }
            int t = -c + L1.getDigit() - L2.getDigit();
            return trimDigitList(new DigitList(t, subDigitLists(0, L1.getNextDigit(), L2.getNextDigit())));
        }
    }
    ...
    public static DigitList digitize(String str) {
        ...
        if (str.indexOf("^") != -1) {
            ...
            L = trimDigitList(L);
            return BigInteger.factorial(new BigInteger(1, L)).getDigits();
        } else {
            DigitList L = null;
